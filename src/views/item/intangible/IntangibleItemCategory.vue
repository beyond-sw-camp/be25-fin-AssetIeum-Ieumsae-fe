...
const categoryResponseId = (response: { categoryId?: string; intangibleAssetCategoryId?: string }) => (
  response.categoryId ?? response.intangibleAssetCategoryId ?? ''
);

const createCategory = async (name: string, parentId: string | null) => {
  const response = await intangibleItemApi.createCategory({
    name,
    parentId,
  });
...
const deleteCategory = async (categoryId: string) => {
  ...
  await intangibleItemApi.deleteCategory(categoryId);
  return true;
};

const emitUpdatedCategories = () => {
...
const addCategory = async () => {
  ...
  emitUpdatedCategories();
  } catch (error) {
    console.error('무형자산 카테고리 등록 실패', error);
...
const deleteMainCategory = async (mainCategory: string) => {
  ...
  emitUpdatedCategories();
    ...
    console.error('무형자산 카테고리 삭제 실패', error);
